...
def add_file(name_data, filename):
    ...
    for line in filename:
        ...
        if len(filename_list) != 1:
            rank = filename_list[0]
            name1 = filename_list[1]
            name1 = name1.strip()
            name2 = filename_list[2]
            name2 = name2.strip()
...
def search_names(name_data, target):
    ...
    for i in range(len(name_list)):
        if target in name_list[i] or target[0].upper() + target[1:] in name_list[i]:
            matching_names.append(name_list[i])
